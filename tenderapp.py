...
# --- 2. HIDE STREAMLIT BRANDING (The Ghost Mode) ---
hide_st_style = """
            <style>
...
class PDF(FPDF):
    def header(self):
        ...
        self.line(10, 20, 200, 20) # Draw a line

    def footer(self):
        self.set_y(-15)
        self.set_font('Arial', 'I', 8)
        self.cell(0, 10, f'Generated by TenderAI | Page {self.page_no()}', 0, 0, 'C')
...
with st.sidebar:
    ...
    st.markdown("---")
    st.caption("v2.0 | Secured by Google Cloud")
...
if st.session_state.myfile is None:
    ...
    # 3. The Dashboard (Only visible after upload)
    ...
    with tab1:
        ...
        if st.session_state.summary:
            ...
            st.info("Click below to extract key dates, EMD, and financial requirements.")
            if st.button("🚀 Generate Summary", type="primary"):
                with st.spinner("Analyzing 50+ pages..."):
    ...
    with tab2:
        ...
        if st.session_state.compliance:
            ...
            st.info("Scan the document for all technical and financial eligibility criteria.")
            if st.button("🔍 Scan for Compliance", type="primary"):
                with st.spinner("Scanning for clauses..."):
    ...
    with tab3:
        ...
        if st.session_state.letter:
            ...
            st.info("Draft a formal covering letter based on standard Indian bidding formats.")
            if st.button("📝 Write Letter", type="primary"):
                with st.spinner("Drafting legal text..."):
    ...
    with tab4:
        ...
        with col_q:
            user_q = st.text_input("Ask a specific question about this tender:", placeholder="e.g., Is Joint Venture allowed?")
        with col_btn:
    ...
    with tab5:
        ...
        with col_d1:
            st.write("Generate a professional PDF containing all the analysis, tables, and the draft letter.")
        
        with col_d2:
            if st.session_state.summary and st.session_state.compliance and st.session_state.letter:
                if st.button("📄 Compile PDF Report", type="primary", use_container_width=True):
                ...
                st.warning("⚠️ Complete the analysis (Tabs 1, 2, & 3) before generating the report.")
